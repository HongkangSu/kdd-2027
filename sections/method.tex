...
\begin{figure*}[t]
  ...
  \caption{Overview of UrbanDreamer: simulator episodes train the flow-matching world model and the HDV response predictor, and the actor is optimized in imagined rollouts with per-vehicle and global critics.}
...
At the macroscopic level of Section~\ref{sec:wm-formulation}, a stack of
graph convolution layers with residual connections and layer
normalization maps the traffic field \(X_t\) (Eq.~\ref{eq:traffic-field})
...
The macroscopic action of Section~\ref{sec:wm-formulation} must condition a
graph-level transition, but decisions are made at the vehicle level.  We
therefore project the realized one-hot CAV route decisions onto the segment
nodes those routes are expected to visit, producing the planned pressure
field \(\Psi_t^{\mathrm{CAV}}\) defined on the
...
The macroscopic transition \(F_\phi\) of Eq.~\ref{eq:macro-transition} is
learned with state-to-state flow
...
The macroscopic transition of Eq.~\ref{eq:macro-transition} is conditioned
on the HDV traffic state, which evolves on two time scales: HDVs choose
routes at departure time and adapt across episodes to CAV-induced
congestion.  UrbanDreamer models the two scales separately.  Within an
episode, the learned transition responds to the current CAV pressure
\(\Psi_t^{\mathrm{CAV}}\) at every decision epoch; across episodes, slower
human adaptation is an episode-level response field over road segments,
predicted from the previous episode and held fixed within each imagined
episode.  Imagined rollouts therefore optimize against step-level
counterfactual traffic responses, while day-to-day HDV demand shifts are
learned from real simulator episodes.
